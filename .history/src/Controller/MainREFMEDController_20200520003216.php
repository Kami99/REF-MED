...
class MainREFMEDController extends AbstractController
{
    ...
    public function index(Request $request)
    {
        ...
        $repodoctor= new DoctorRepository();
        if($formSearchDoctor->isSubmitted() && $formSearchDoctor->isValid()){
           ...
           if($data['lat'] == null or $data['lon'] == null)
           {
               ...
               while ($row=$result->fetch()) {
                   $doctors[]= $repodoctor->find($row['id']);
                   
                   dd($doctors);
                   $doctor->setEmail($row['email']);
                   $doctor->setFirstName($row['first_name']);
                   $doctor->setLastName($row['last_name']);
                   $doctor->setNumberPhone($row['number_phone']);
                   $doctor->setProfession($row['profession']);
                   $doctor->setTarifMin($row['tarif_min']);
                   $doctor->setTarifMax($row['tarif_max']);
                   $doctor->setSlug($row['slug']);
                   $doctor->setPictureProfil($row['picture_profil']);

    ...
    public function index(Request $request)
    {
        ...
        if($formSearchDoctor->isSubmitted() && $formSearchDoctor->isValid()){
           ...
           if($data['lat'] == null or $data['lon'] == null)
           {
               ...
               $sql="SELECT doctor.id,doctor.first_name,doctor.last_name, location.name_location, location.latitude, location.longitude, ( 6371 * acos( cos( radians( 14.6937) ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians(-17.44406) ) + sin( radians(14.6937) ) * sin( radians( latitude ) ) ) ) AS distance ,profession.profession_name AS profession FROM doctor INNER JOIN profession INNER JOIN location ON doctor.profession_id = profession.id AND doctor.location_id = location.id WHERE profession.profession_name = '{$data['profession']}' HAVING distance < {$data['distance']} ORDER BY distance";
               $result=$entityManager->getConnection()->prepare
           }
           else {

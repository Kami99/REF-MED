...
class MainREFMEDController extends AbstractController
{
    ...
    public function index(Request $request)
    {
        ...
        $formSearchLaboratory->handleRequest($request);
        if($formSearchDoctor->isSubmitted() && $formSearchDoctor->isValid()){
           ...
           if($data['lat'] == null or $data['lon'] == null)
           {
               ...
               ( 6371 * acos( cos( radians({$this->dataLat}) ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians({$this->dataLon}) ) + sin( radians({$this->dataLat}) ) * sin( radians( latitude ) ) ) ) AS distance ,
        ...
        if($formSearchLaboratory->isSubmitted() && $formSearchLaboratory->isValid()){
            ...
            $this->distance=null;
            $userDistance=$data['distance'];    
            dd($data['categorie']->getNameCategory());
            if($data['lat'] == null or $data['lon'] == null)
            {
                $entityManager = $this->getDoctrine()->getManager();
                $sql="SELECT users.email, laboratory.id,laboratory.name_laboratory, laboratory.number_phone,
                 location.name_location, location.latitude, location.longitude, 
                 ( 6371 * acos( cos( radians({$this->dataLat}) ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians({$this->dataLon}) ) + sin( radians({$this->dataLat}) ) * sin( radians( latitude ) ) ) ) AS distance,
                 category_laboratory.name_category AS category_laboratory FROM laboratory  
                 INNER JOIN users INNER JOIN category_laboratory INNER JOIN laboratory_category_laboratory INNER JOIN location 
                 ON category_laboratory.id = laboratory_category_laboratory.category_laboratory_id 
                 AND laboratory_category_laboratory.laboratory_id = laboratory.id 
                 AND laboratory.location_id = location.id
                  WHERE AND users.id = doctor.id category_laboratory.name_category 
                  LIKE '%{$data['categorie']->getNameCategory()}%' HAVING distance < {$data['distance']} ORDER BY distance";
                $result=$entityManager->getConnection()->prepare($sql);
                $result->execute();
                while ($row=$result->fetch()) {

        ...
        if($formSearchDrugs->isSubmitted() && $formSearchDrugs->isValid()){
            ...
            if($data['lat'] == null or $data['lon'] == null)
            {
                ...
                $sql="SELECT pharmacy.id,pharmacy.pharmacy_name,pharmacy.phone_number,
                 location.name_location, location.latitude, location.longitude, ( 6371 * acos( cos( radians( 14.6937) ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians(-17.44406) ) + sin( radians(14.6937) ) * sin( radians( latitude ) ) ) ) AS distance
                  FROM pharmacy INNER JOIN pharmacy_availabe_drugs INNER JOIN drugs INNER JOIN location ON pharmacy_availabe_drugs.pharmacy_id = pharmacy.id AND pharmacy.location_id = location.id AND pharmacy_availabe_drugs.drugs_id = drugs.id WHERE drugs.name_drugs LIKE '%mas%' AND pharmacy_availabe_drugs.is_available = 1 HAVING distance < 50 ORDER BY distance";
                $result=$entityManager->getConnection()->prepare($sql);
                $result->execute();
                while ($row=$result->fetch()) {

        ...
        if($formSearchDoctor->isSubmitted() && $formSearchDoctor->isValid()){
           ...
           if($data['lat'] == null or $data['lon'] == null)
           {
               ...
               while ($row=$result->fetch()) {
                   ...
                   $doctor->setFirstName($row['email']);
                   $doctor->setLastName($row['email']);
                   $doctor->setNumberPhone($row['email']);
                   $doctor->setProfession($row['email']);
                   $doctor->setTarifMin($row['email']);
                   $doctor->setTarifMax($row['email']);
                   $doctor->setSlug($row['email']);
                   $doctor->setPictureProfil($row['email']);

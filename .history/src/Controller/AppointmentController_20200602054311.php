...
{
    /**
     * @Route("/rdv-non-confirme", name="appointment_unconfirmed")
     */
    public function unconfirmedAppoint(AppointmentRepository $appointmentRepository, Request $request): Response
    {
        ...
        if ($formAppoint->isSubmitted() && $formAppoint->isValid()) {
           $data=$formAppoint->getData();     
            ...
            $this->addFlash(
                'success',
                "Votre rendez vous a bien été enregistrées si vous voulez le confirmé veuillez partir dans la partie gestion"
            );
         return $this->redirectToRoute('appointment_calendar');  

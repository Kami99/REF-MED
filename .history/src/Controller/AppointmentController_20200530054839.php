...
class AppointmentController extends AbstractController
{
     ...
     * @Route("/{id}", name="appointment_delete", methods={"DELETE"})
     */
    public function allShowAppoint(Request $request, Appointment $appointment): Response
    {
        if ($this->isCsrfTokenValid('delete'.$appointment->getId(), $request->request->get('_token'))) {
            $entityManager = $this->getDoctrine()->getManager();
            $entityManager->remove($appointment);
    ...
    /**
     * @Route("/rdv-non-confirme", name="appoint_not_confirmed", methods={"DELETE"})
     */
    public function ShowAppointNotConfirmed(Request $request, Appointment $appointment): Response
    {
        $doctor=$this->getUser();
        return $this->render('appointment/index.html.twig', [
            'appointments' => $appointmentRepository->findAll(),

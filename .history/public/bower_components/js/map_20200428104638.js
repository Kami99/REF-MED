...
document.querySelector("#codePostal").addEventListener("blur", getAdress)
...
function mapClickListen(e) {
    ...
    xmlhttp.onreadystatechange = () => {
        ...
        if(xmlhttp.readyState == 4){
            ...
            if(xmlhttp.status == 200){
                // On récupère la réponselet response = JSON.parse(xmlhttp.response)
    ...
    // On ouvre la requête
    xmlhttp.open('get', `https://nominatim.openstreetmap.org/reverse.php?format=html&lat=${pos.lat}&lon=${pos.lng}`)
...
function getAdress() {
    let adress= "Dakar, "+document.querySelector("#adress").value+" "+document.querySelector("#codePostal").value
    //Initialisation ajax
    const xmlhttp = new XMLHttpRequest
